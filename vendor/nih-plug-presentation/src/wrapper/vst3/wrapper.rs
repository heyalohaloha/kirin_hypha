...
use vst3_sys::vst::{
    kNoParamId, kNoParentUnitId, kNoProgramListId, kRootUnitId, BusDirections, Event, EventTypes,
    IAudioPresentationLatency, IAudioProcessor, IComponent, IEditController, IEventList,
...
impl<P: Vst3Plugin> IAudioPresentationLatency for Wrapper<P> {
        ...
        process_wrapper(|| {
            ...
            let result = loop {
                ...
                let result = if is_param_flush {
                    ...
                    let input_presentation = self
                        .inner
                        .input_presentation_latency
                        .load(Ordering::Acquire);
                    let output_presentation = self
                        .inner
                        .output_presentation_latency
                        .load(Ordering::Acquire);
                    transport.input_presentation_latency_samples =
                        (input_presentation != u32::MAX).then_some(input_presentation);
                    transport.output_presentation_latency_samples =
                        (output_presentation != u32::MAX).then_some(output_presentation);
