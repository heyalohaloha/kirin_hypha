...
macro_rules! add_mouse_button_class_method {
    ($class:ident, $sel:ident, $event_ty:ident, $button:expr) => {
        ...
        extern "C" fn $sel(this: &Object, _: Sel, event: id){
            // Kirin Hypha fork (2026-04-18): log raw AppKit mouse events so we can tell
            // whether the first click actually reaches the NSView. Remove after cause 2
            // diagnosis is complete.
            log::info!("[hypha-fork] {} fired: button={:?}", stringify!($sel), $button);

            let state = unsafe { WindowState::from_view(this) };

            let modifiers = unsafe { NSEvent::modifierFlags(event) };

            // Kirin Hypha fork (2026-04-18, cause 2 fix): baseview's MouseEvent::Button{Pressed,
            // Released} carry no position. egui-baseview gates forwarding on
            // `pointer_pos_in_points`, which only gets set by CursorMoved. On macOS, mouseMoved:
            // is not delivered to non-key windows, so the first mouseDown after a window becomes
            // active has no preceding CursorMoved and egui-baseview silently drops the click.
            // Synthesize a CursorMoved from the NSEvent location so egui always has a valid
            // pointer position. Logic mirrors `mouse_moved` (L400-415 in this file).
            let point: NSPoint = unsafe {
                let location = NSEvent::locationInWindow(event);
                msg_send![this, convertPoint:location fromView:nil]
            };
            let synth_position = Point { x: point.x, y: point.y };
            log::info!(
                "[hypha-fork] synthesized CursorMoved before {}: pos=({}, {})",
                stringify!($sel), synth_position.x, synth_position.y
            );
...
// Kirin Hypha fork (2026-04-18, cause 2 follow-up): Apple NSResponder docs state
// that subclass overrides of becomeFirstResponder / resignFirstResponder MUST call
// `[super ...]` to get the default responder-chain bookkeeping. Upstream baseview
// returned YES without invoking super, which we suspect is why AppKit sometimes
// fails to route the first mouseDown: to this view under parented VST3 hosting.
// iPlug2's main view does not override these methods at all and is known to pass
// Studio One's first-click test; matching Apple's required pattern is the safe
// minimum. We also surface super's return value in the log so the next repro run
// can confirm that AppKit accepted the transition.
extern "C" fn become_first_responder(this: &Object, _sel: Sel) -> BOOL {
    let super_ok: BOOL = unsafe {
        let superclass = msg_send![this, superclass];
    ...
    log::info!(
        "[hypha-fork] become_first_responder (is_key_window={}, super={})",
        is_key_window,
        super_ok == YES
    );
    if is_key_window {
...
extern "C" fn resign_first_responder(this: &Object, _sel: Sel) -> BOOL {
    ...
    log::info!("[hypha-fork] resign_first_responder (super={})", super_ok == YES);
    let state = unsafe { WindowState::from_view(this) };
...
// Kirin Hypha fork (2026-04-18, cause 2 follow-up): match the iPlug2 pattern
// (IGraphicsMac_view.mm L631-670) by doing `makeFirstResponder` and
// `setAcceptsMouseMovedEvents` in `viewDidMoveToWindow` (after the view is in
// the window hierarchy) rather than `viewWillMoveToWindow:` (before). The prior
// ordering occasionally left the view first-responder-less in Studio One, which
// may have contributed to mouseDown: never reaching our NSView on first click.
extern "C" fn view_did_move_to_window(this: &Object, _self: Sel) {
    unsafe {
        let window: id = msg_send![this, window];
        if window != nil {
...
// Kirin Hypha fork (2026-04-18, cause 2 follow-up): iPlug2's IGraphicsMac_view.mm
// calls `[super updateTrackingAreas]` first with the comment
// "This is needed to get mouseEntered and mouseExited". Upstream baseview skipped
// the super call, which may leave AppKit's internal tracking bookkeeping stale
// after window-tree changes and contribute to missed mouse events on first click.
extern "C" fn update_tracking_areas(this: &Object, _self: Sel, _: id) {
    unsafe {
        let superclass = msg_send![this, superclass];
        let () = msg_send![super(this, superclass), updateTrackingAreas];
