    ...
    U: 'static + Send,
{
    ...
    fn on_event(&mut self, _window: &mut Window, event: Event) -> EventStatus {
        ...
        let is_keyboard = matches!(event, baseview::Event::Keyboard(_));
        match &event {
            baseview::Event::Mouse(event) => match event {
                ...
                baseview::MouseEvent::ButtonPressed { button, modifiers } => {
                    ...
                    // Kirin Hypha fork (2026-04-18, cause 2 verification): log whether the
                    // ButtonPressed actually reaches egui. Paired with baseview's synthesized
                    // CursorMoved in add_mouse_button_class_method! so we can tell if cause 2
                    // fix landed. Remove once verification is complete.
                    if let Some(pos) = self.pointer_pos_in_points {
                        log::info!(
                            "[hypha-fork] PointerButton pressed forwarded to egui: pos=({}, {})",
                            pos.x, pos.y
                        );
                        ...
                        log::info!(
                            "[hypha-fork] PointerButton pressed DROPPED (pointer_pos=None)"
                        );
                    }
                }
                baseview::MouseEvent::ButtonReleased { button, modifiers } => {
                    ...
                    // Kirin Hypha fork (2026-04-18, cause 2 verification): see ButtonPressed
                    // above. Remove once verification is complete.
                    if let Some(pos) = self.pointer_pos_in_points {
                        log::info!(
                            "[hypha-fork] PointerButton released forwarded to egui: pos=({}, {})",
                            pos.x, pos.y
                        );
                        if let Some(button) = crate::translate::translate_mouse_button(*button) {
                            self.egui_input.events.push(egui::Event::PointerButton {
                        ...
                        log::info!(
                            "[hypha-fork] PointerButton released DROPPED (pointer_pos=None)"
                        );
                    }
                }
