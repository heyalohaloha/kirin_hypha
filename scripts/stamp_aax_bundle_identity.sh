...
[[ $# -eq 4 ]] || {
  echo "usage: stamp_aax_bundle_identity.sh BUNDLE IDENTITY_HEADER KIMERA_EMBEDDED DISTRIBUTION_BUILD" >&2
  exit 2
}
...
[[ "$distribution_build" == ON || "$distribution_build" == OFF ]] \
  || { echo "invalid AAX distribution build value: $distribution_build" >&2; exit 1; }
...
/usr/libexec/PlistBuddy -c "Add :KirinHyphaAaxBuildMode string $([[ "$distribution_build" == ON ]] && echo release || echo diagnostic)" "$plist"
