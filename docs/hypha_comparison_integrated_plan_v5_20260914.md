...
作成日: 2026-09-14。
改訂: v5 revision 3。B-890とW-3083のレビュー後の構造修正計画を追加。
状態: B-890とW-3083の実装後レビューで未解決項目あり。修正の実装と受入は未完了。
本改訂は計画のみを変更する。後続commitのDAWへの配置、実機受入、公開が完了したことを示さない。
...
[B-890とW-3083の構造修正計画](reference_b890_structural_repair_plan_20260914.md)を本書の修正工程として追加する。
レビュー7件、同じ責務の容量と寿命、回復、追加負荷は同計画で扱い、該当範囲では同計画の手順を優先する。
利用者の指示により、今回の旧reader互換試験と互換fallbackの維持義務を外す。
既存原本とIDの保全、namespaceによる障害分離、音声契約、共通予算と完成状態は維持する。
第4節のB-887とW-3080は当初の基点記録であり、修正工程は同計画に記したB-890とW-3083から開始する。
