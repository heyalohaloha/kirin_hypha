...
| INV-S29 | 表示専用filterはliveのWatch数値セルとFREQ live curveの2つだけで、どちらも描画だけを変え、計測・保存・読み戻しの値を変えない。Watchのcurrent 6セル（LUFS-M／直近True Peak／Crest／PSR／N／Sharpness）とPOST Δは1.5秒時定数で追従し、True Peakの上昇だけはその観測で即時表示する。観測間隔は1秒で頭打ちにし、最後の観測から5秒で減光、9秒で保持を終える。`tp_session_max`、LUFS-S、`psb_low`／`psb_mid`／`psb_high`、`n_prime[20]`、`psb_bark[20]`、`dropped_samples`はfilterを通さず素通しする。playback-pass／Keep MAX、Meter Session、TIME履歴、Record、Keep、`plugin_data`、FREQ数値readout、MARK、Focus Trail、peak holdも同じく素通しする。「値なし」は0.0ではなくNaNのまま運ぶ。この2つの存在と範囲を`README.md`の Design 節に明記し、どちらも無いとは書かない | `juce_shell/tests/display_smoothing_contract_test.cpp` / `session_maximum_is_never_smoothed_toward_a_value_it_never_had` / `short_term_loudness_passes_through_as_the_exact_three_second_window` / `verifySpectrumPresentationContract` |
