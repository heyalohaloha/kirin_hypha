...
| INV-S34 | 200%と300%のPOST絶対FREQは、六秒FIELDの代わりに同じ180 frameを遠近の地形（ridge-line）として描く。最新のridgeは平面plotそのもので、xはcurveと同じ`(index+0.5)/256`、yは同じ0〜−96 dBFSとする。古いridgeほど上、中央寄り、細く、薄く描き、奥行きの縮尺は`1/(1+2.4×年齢/6秒)`、最古のridgeの床はplot上端から34%の高さ、0 dBFSの山もplot内に収める。24本のridgeはそれぞれ自分の年齢に最も近い実測frameを1枚だけ描き、そのframeが半ridge間隔（約0.13秒）より遠いridgeは描かない。時間方向の格子線は隣り合うridgeの間だけに引くので、欠測は空白のまま残り、離れた2観測の間に山を作らない。周波数方向の96列は列内に中心を持つbandの最大値を取り、狭いpeakを間引かない。ridgeの下の暗い覆いは奥を隠すためだけの面で値を持たない。地形は値を加えず、現在の値は上に重なる平面curveが示す。PRE/POST対応のΔ表示とM/Sは平面のまま地形を描かない。同じ2サイズではplotの四隅の目印、描いている量の定義（Δは`Δ(f) = POST(f) - PRE(f)  dB`、POST絶対は`L(f) = POST(f)`）、表示中の解析条件（APERTURE ms、FFT点数、256 BANDS、30 Hz）をsnapshotの値からplot左下へ描き、M/Sには描かない。Audio Threadの仕事を増やさず、既存のpaint上限を超えない | `verifyLevelLandscape` / `verifyAbsoluteSpectrumContract` / `juce_shell/tests/ui_render_contract_test.cpp` |
