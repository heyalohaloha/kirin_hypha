...
| INV-S25 | ローカルBlindの製品UIはPOSTの大画面で一つだけ入口を持ち、PREは別の解析枠を消費しない。取得、準備、固定減衰の明示承認、Source 1／2の完全試聴、回答、Reveal、中断、Audio Thread確認後の通常復帰を同じ全画面UIで進める。Reveal前は割当を表示せず、背後のmeter、pair、Capture、Reference、情報操作をmouse、keyboard、accessibilityから隔離する。回答は両側の完全な一巡後だけ許可する。300×200への縮小、editor再表示、途中終了でも操作を失わず、自動再生、自動Reveal、自動通常復帰を行わない。Windows VST3、macOS VST3、macOS AUのexact 4秒PDC実証完了を受け、単一gateを既定ONにする | `verifyLocalBlindUiContract` / `kirin_product_entry_contract` / `juce_shell/src/PluginEditorLocalBlind.cpp` |
