...
| INV-S14 | ATTACKはTRACK／STEMのDRUMに限定したPOST on-demand routeである。HISTORYは同一exact content event系列のPRE traceとPOST bodyを六秒の同一固定尺度（−72..0 dBFS）へ描く。その下に同じ時間軸と横plot列を共有するTRANSIENT（30ms attack RMS − 直前100ms context RMS）、STRENGTH（30ms attack RMS）、CREST（30ms内sample peak − attack RMS）、SHARPNESS（既存100ms acum）の四laneを置く。pair時は同じonset sampleのPRE/POST detailだけから打音ごとの符号付き`POST−PRE`を固定尺度（dB laneは共通±12 dB、SHARPNESSは±1 acum、範囲外はcap付きclip）で描き、PRE未接続時はPOST絶対値（TRANSIENT 0–18 dB、STRENGTH −72..0 dBFS、CREST 0–24 dB）へ切り替え、per-hit SHARPNESSは差分だけを表示する。PRE/POSTのonset sampleが異なる打音、直前contextがHISTORY下限未満のTRANSIENT、PRE-only／POST-only／ambiguous、欠測は値もΔも出さず理由を示す。lane readoutと一行表示もlaneと同じ量（pair時は差分、PRE未接続時はPOST値）だけを示し、打音ごとのPRE値・POST値を並べない。選択打音は静的な菌糸線一本（exact x ±1 px）で示し、値・loupe・一行表示は六秒窓内の選択打音だけを示す（窓外へ出たLOCK打音は示さない）。Inspectionだけ選択打音の130ms窓をPRE/POST shapeとRMS段で拡大し、100%／125%はHISTORYと選択打音の四値一行へ畳む。楽器分類、採点、推奨、相関shift、補間を行わない。旧`AnalysisViewMode`値、既存FFI layout／transport、二枠lease coordinatorを維持し、Audio Threadの仕事を増やさない。2MIX用ATTACKは別の精度契約が成立するまでこのrouteへ接続しない | `docs/hypha_drum_lanes_20260924.md` / `juce_shell/src/HyphaAttackUiContract.h`の`static_assert` / ATTACK analyzer・alignment・transport tests / `juce_shell/tests/AttackUiContractTest.cpp` |
