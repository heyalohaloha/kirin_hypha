...
| INV-S14 | ATTACKはTRACK／STEMのDRUMに限定したPOST on-demand routeである。HISTORYは同一exact content event系列のPRE traceとPOST bodyを六秒の同一固定尺度へ描く。選択eventのTRANSIENTは30ms attack RMSと直前100ms context RMSの局所contrastをPRE、POST、符号付き`POST−PRE`で表示し、barだけを0–18 dBへclipする。中央標本はPOST絶対値だけをStrength（30ms attack RMS）、Texture（sample-edge比／Crest／peak plateauの複合量）、Sharpness（既存100ms acum）へ独立写像し、PRE、Transient、六秒waveform、表示用motionで形を変えない。楽器分類、採点、推奨、相関shift、補間を行わず、欠測、PRE-only、POST-only、無音を区別する。旧`AnalysisViewMode`値、既存FFI layout／transport、二枠lease coordinatorを維持し、Audio Threadの仕事を増やさない。2MIX用ATTACKは別の精度契約が成立するまでこのrouteへ接続しない | `docs/hypha_attack_visual_completion_proposal_20260909.md` / ATTACK analyzer・alignment・transport tests / `juce_shell/tests/AttackUiContractTest.cpp` |
