...
| INV-D2 | POSTがActiveのままpaired PREだけInactiveになった場合、pairを外さず`PreInactive`へ遷移する。停止・無音・stale・一時不在を明示OFFと推測せず、理由付き比較snapshotをLEVEL／TIME／FREQへ同じ語彙で渡す | `latch_inactive_switches_to_post_absolute_without_releasing_pair` / `comparison_state_drives_the_shipping_observatory_without_a_hidden_delta_path` / `juce_shell/tests/ComparisonPresentationContractTest.cpp` |
...
| INV-D10 | paired PREの確定`Bypassed`だけがpairを保持したままPOST絶対値へ戻し、右上を淡い紫のASCII `ABS`にする。`PreInactive` / Stale / NoPre / poll失敗ではOFFを推測せず、保持値が実在する場合だけ`Holding`を表示する | `comparison_state_drives_the_shipping_observatory_without_a_hidden_delta_path` / `juce_shell/tests/ComparisonPresentationContractTest.cpp` / `juce_shell/tests/ui_render_contract_test.cpp` |
...
| ID | 不変条件 | 紐づくテスト |
|----|----------|--------------|
| INV-I1 | `setStateInformation` がenable前後どちらでもPREの任意名を反映する。POSTは保存済みexact PRE locatorと表示名を一回で復元し、名前だけの旧stateから自動ペアしない | `restored_pre_name_before_enable_is_written_to_pre_watch_json` / `restored_pre_name_after_enable_updates_pre_watch_json`（`#[ignore]`）/ `exact_restore_keeps_optional_name_as_metadata` / `saved_daw_state_restores_the_exact_pre_without_registry_rescan` |
...
| INV-I5 | VST3 hostの明示component OFFは`IComponent::setActive(false)`専用通知で保持し、engine生成前の通知もfresh handleへ再適用する。heartbeatが100 ms監視の4 tick（400 ms）staleになった場合だけ`Bypassed`へ確定する。これはAudio callback停止の鮮度境界であり、callbackが継続する実際の無音は別の3秒silence gateで扱う。通常停止・無音・一時再構成は`Inactive`、generic `releaseResources()`は利用者OFFを発明しない。reactivate後はprocess再開まで`Inactive` | `b118_live_window_is_four_ticks_400ms` / `b544_host_component_state_only_changes_stalled_signal_reason` / `vst3_component_activation_is_distinct_from_release_resources` |
...
| INV-S4 | POST Record表示は世代付きsnapshotを使い、host inactive／finalize／Stop後もObservatoryに6つの結果factを保持する。Max TP/Iは絶対値、選択M/S・PSR・Crest・SharpだけがΔ対象で、Captureも同じsnapshotを複製する | `juce_record_result_is_presented_by_the_observatory_and_capture` / `juce_shell/tests/ComparisonPresentationContractTest.cpp` |
...
| INV-S6 | 出荷AU/VST3は同一JUCE processor/editor/Observatory sourceと単一`HyphaUiContract`をcompileする。300×200矩形、全bounds、font family/size、palette、Watch、Record、Keep/Stop、ABI fail-closed判定をbundle build前にpure C++ testで固定する。非表示のMetricCell／PostControlsを出荷状態の代用にしない | `shipped_au_and_vst3_compile_the_same_editor_processor_and_control_contract` / `comparison_state_drives_the_shipping_observatory_without_a_hidden_delta_path` / `juce_shell/tests/ui_contract_test.cpp` |
| INV-S7 | POSTのoptional analysisはMeters=全停止、ATTACK=paired event、FREQ=FFT、SHARP=exact pair成立時はSharpness差分・未成立時はlocal POST Sharpness絶対値、LIVE=local POST三指標絶対値のinstance内排他状態。SHARPの単体表示はLIVEと同じabsolute timelineを使い、PRE要求を生成しない。pair成立・解除時は同じ画面内で解析modeと見出し・固定scaleを切り替える。DAW process内のstable kernel leaseは2枠で、3個目は実際のlease保持pairを一行の`ANALYSIS IN USE / Mix, Vocal`で表示して解析しない。2名を検証できない場合は`ANALYSIS IN USE`へfail closedし、推測名や閉じる命令は出さない。ATTACK/FREQ/SHARP/LIVE切替は同じ枠を保持するが、利用者がMETERSへ戻す、editorを閉じる、またはhostがeditorを破棄せず非表示にした時点で全optional leaseを解放する。再表示時は現在ページのleaseだけを再取得し、待機中instanceが自動取得できる。TIMEはObservatoryで`HISTORY / ATTACK / SHARP / LIVE`を直接選択し、Compactでは一つのcycle controlへ畳む。FREQは一階層目のdomainに保つ。切替tooltipはASCII sentenceで固定し、Audio Threadは既存のlock-free copy以上を行わない | `exactly_two_post_analysis_runtimes_are_active_per_process_lease` / `analysis_owner_names_are_bounded_utf8_and_null_terminated` / `juce_shell/tests/ui_render_contract_test.cpp` / `verifyTimePageNavigationContract` / `perceptual_mode_is_exclusive_and_publishes_exact_100ms_apertures` / `post_absolute_timeline_needs_no_pair_and_never_creates_a_pre_request` / `disabled_runtime_does_not_start_worker_or_accept_audio` / `process_block_calls_only_rt_safe_ffi_surface` / `verifyPerceptualHistoryContract` / `verifyAbsoluteTimelineContract` |
| INV-S8 | SHARPの差分表示はRust側の最新64実測点を非破壊batchで取得し、UI遅延で欠測を作らない。UIは60点/六秒、curve 5 Hz、数値 2 Hz。カーブは時間方向に同じ濃度。塗りの濃度は履歴の先頭・時刻・停止状態ではなく0線からの距離だけで決まり、正負とも0で薄く表示端へ向かうほど濃くなる。最新の実測点だけをドットで示す。POST単体表示はabsolute timelineのSharpnessだけを0..3 acum固定scaleで描く。どちらも真の欠測を補間せず、前の表示runを捨てて新しく始める | `perceptual_join_recovers_every_exact_endpoint_after_a_delayed_presentation_tick` / `repeated_sixteen_frame_exchange_windows_accumulate_without_rewinding` / `verifyPerceptualHistoryContract` / `verifyPerceptualRenderingContract` / `verifyAbsoluteTimelineContract` |
...
| INV-S14 | ATTACKはTRACK／STEMのDRUMに限定したPOST on-demand routeである。HISTORYは同一exact content event系列のPRE traceとPOST bodyを六秒の同一固定尺度（−72..0 dBFS）へ描く。その下に同じ時間軸と横plot列を共有するTRANSIENT（頭30ms RMS − 後続body RMS。bodyは頭の後100msか次onsetまで）、STRENGTH（頭30ms RMS）、CREST（頭30ms内sample peak − 頭RMS）、SHARPNESS（頭から100msのDIN 45692音量加重平均）の四laneを置く。全ての窓はonsetを含む約1msのcontent binから始まる（B-1016）。打音は頭30msが揃った時点でSTRENGTH／CRESTだけの頭detailとして出し、body・次onset・Sharpness窓が確定したら同じ打音の完全なdetailへ置き換える（B-1024）。再生停止で音声が来なかった打音は頭detailのまま残り、TRANSIENT／SHARPNESSを窓の短縮や無音の補填で作らない。shapeは測定済みの区間だけを持つ。pair時はPOSTをPREのonsetとPREのbody終端で測り直し、同じcontent sampleどうしの打音ごとの符号付き`POST−PRE`を固定尺度（dB laneは共通±12 dB、SHARPNESSは±1 acum、範囲外はcap付きclip）で描き、PRE未接続時はPOST絶対値（TRANSIENT −12..24 dB、STRENGTH −72..0 dBFS、CREST 0–24 dB、SHARPNESS 0–8 acum）へ切り替える。bodyが次onsetで20ms未満になるTRANSIENT、bodyがHISTORY下限未満のTRANSIENT、PRE-only／POST-only／ambiguous、欠測は値もΔも出さず理由を示す。lane readoutと一行表示もlaneと同じ量（pair時は差分、PRE未接続時はPOST値）だけを示し、打音ごとのPRE値・POST値を並べない。選択打音は静的な菌糸線一本（exact x ±1 px）で示し、値・loupe・一行表示は六秒窓内の選択打音だけを示す（窓外へ出たLOCK打音は示さない）。Inspectionだけ選択打音の150ms窓（onsetの20ms前からbody終端まで）をPRE/POST shapeと頭・bodyのRMS段で拡大し、100%／125%はHISTORYと選択打音の四値一行へ畳む。楽器分類、採点、推奨、相関shift、補間を行わない。旧`AnalysisViewMode`値、FFI struct size、二枠lease coordinatorを維持し、Audio Threadの仕事を増やさない。PRE→POST ATTACK snapshotは版3で、版の違うsnapshotは読まない。2MIX用ATTACKは別の精度契約が成立するまでこのrouteへ接続しない | `docs/hypha_drum_lanes_20260924.md` / `juce_shell/src/HyphaAttackUiContract.h`の`static_assert` / ATTACK analyzer・alignment・transport tests / `per_hit_sharpness_follows_the_onset_not_a_fixed_grid` / `host_block_size_never_changes_a_value` / `the_body_ends_at_the_next_queued_onset` / `a_hit_whose_audio_stops_keeps_its_head` / `a_complete_detail_replaces_its_head_and_advances_the_revision` / `drum_ui_windows_are_the_measurement_windows` / `exact_pair_transports_real_pre_and_post_attack_histories_end_to_end` / `juce_shell/tests/AttackUiContractTest.cpp` |
| INV-S15 | 常設Meter SessionはRecord／Keep／Watch pass／pairing／Guide／editor lifetimeから独立し、Active音声だけで進む。Inactive／Bypassedは保持したままPauseし、明示Resetだけがgenerationを進めて統計と履歴を破棄する。Hybrid VUの明示`CLEAR`はL/R保持TPとVU専用clip表示ラッチだけを解除し、current、VU／TP窓、Sample Peak hold、I／LRA／MaxTP／PLR、L/R Session累積clip event、TIME履歴、Record／Keepを維持する。解除中もclipが続けば次の100ms観測で表示だけ再点灯し、同じ連続runをSessionへ二重算入しない。current、10ms規格解析のMax M、session、L/R Peak／TP、3秒BAL／CORRは同じ100ms公開境界を持つ。clipはチャンネル別`abs(sample) >= 1.0`の連続runを1 eventとし、100ms境界で分割しない。Measure worker再起動とRecord pre-roll replayでResetや二重算入を起こさず、Audio Threadには新しいlock／alloc／copyを追加しない | `starts_empty_and_only_active_audio_advances_time` / `maximum_momentary_is_an_official_session_fact_until_explicit_reset` / `integrated_lra_peak_and_plr_share_one_session_fact` / `clear_peak_clip_holds_preserves_live_windows_and_relatches_continuing_clip` / `peak_clip_clear_does_not_discard_meter_session_truth` / `in_phase_inverse_and_balance_share_exact_three_second_window` / `one_sided_and_mono_are_explicit_not_invented_numeric_stereo` / `clip_events_are_channel_specific_contiguous_runs_across_observations` / `live_measure_worker_advances_pauses_and_resets_independent_session` / `drain_includes_tail_off_undercounts_on_matches_full` / `b118_measure_restart_recovers_via_watchdog` / `process_block_calls_only_rt_safe_ffi_surface` |
...
| INV-S17 | TIME Δはexact PREの現runtime ownerと専用`meter_history.json`を照合し、同一sample rate・**同一測定配置（`MeasurementLayout`: layout名・役割並び・loudness map・map規則版）**・presentation source・一意sample endpointの100ms点だけをPOST−PREする。違う map で測った2本は引き算しない（mono PRE と stereo POST は同じ音でも3.01 LUずれ、その差は連鎖が加えたものではない / B-968）。PRE直近32点／POST直近64点の固定窓、pair/runtime/reset境界で履歴を破棄し、欠測・重複endpoint・transport jumpを補間せず別runにする。集約はexact Δの後に行い、PRE/POST集約値同士を減算しない。Audio Threadは変更しない | `atomic_publication_and_exact_target_join_work_end_to_end` / `joins_only_the_same_unique_presentation_endpoint` / `repeated_or_missing_endpoints_never_create_a_delta_fact` / `pair_change_discards_history_instead_of_blending_sources` / `a_different_layout_is_not_subtracted_because_the_gap_is_the_map_not_the_chain` / `delta_history_abi_is_post_only_and_empty_is_a_valid_fact` / `verifyTimeHistoryContract` |
...
| INV-S21 | ローカルBlindのPRE PCMは、exact request全体のdigest、pair／capture／clock generation、PRE native範囲、形式、期限、PCM全体のSHA-256へ固定した一回限りの不変artifactとして非RTで運ぶ。期限は新規ownerの受付／armに適用する。期限前のexact PRE armed証拠がありAudio Thread上で全範囲を完了したPCMは、同じ現pair claimを保つ間、非RT回収が期限後でもfinalizeできる。期限後の新規arm、未完了capture、arm証拠なしは拒否する。PRE laneが失敗した場合は同じrequest digestと全世代に固定したterminal失敗artifactを返し、完成PCMの公開が連続失敗した場合も有限回でterminal化する。同一要求の成功と失敗は同時に公開しない。POSTは成功または失敗のterminal結果を検証し、失敗時は待機を終了して理由を保持する。PREがどちらも返さない場合はPOSTのcapture完了から独立した30秒のfinalization期限で失敗を確定し、受付期限と混同しない。成功時は全artifactをPOST所有storageへコピーし、role-local POST receiptと同じpair barrierが完成した後だけexactな消費応答を返す。PREはその応答とAudio Thread reader退場の両方を確認してからlocal PCMとartifactを破棄する。欠損、改変、短いbuffer、非有限値、旧pair、世代／範囲不一致は出力無変更で拒否し、pair変更はbarrier完成後も保持結果を失効させる。このbarrier単体では試聴を開始しない | `exact_pre_pcm_and_receipt_round_trip_before_consumption` / `exact_pre_failure_round_trips_after_admission_deadline` / `pre_failure_rejects_unarmed_invalid_and_mismatched_requests` / `one_request_cannot_publish_both_terminal_results` / `immutable_identity_rejects_different_pcm_and_tampering` / `malformed_missing_and_stale_pair_results_fail_closed_after_admission` / `result_transport_requires_the_exact_pre_arm_proof` / `exact_unnamed_pair_completes_the_local_blind_request_handshake` / `juce_shell/tests/local_blind_capture_service_test.cpp` / `local_blind_capture_binds_the_existing_exact_pair_without_requiring_a_name` |
| INV-S22 | ローカルBlindの取得範囲はPOSTのlive host clockから一度だけ将来の単一native範囲として作り、発行位置とともにPRE／POSTへ同じ値を配る。presentation latencyを足し引きして別範囲を捏造しない。各roleのAudio Threadは最初のcallbackを発行位置から取得開始までに限定し、以後の位置連続性、clock source、optional presentation通知を固定する。late arm、発行後の巻戻し、seek、loop、clock／latency変更、timeline停止callback、bypass、offlineは当該要求だけの失敗にし、失敗状態と理由は明示resetまで保持する。出荷Releaseで要求計画用clock probeを常時書込むのはPOSTだけとし、Debug診断は両roleを観測できる。Debugの明示取得は完成PCMを非RTで比較するだけで、範囲補正、開始許可、試聴出力へ接続しない。既定OFFの検証VST3は別identityで4096 samplesの実遅延とhost報告値を一致させる。Windows Studio Pro、macOS Studio ProのVST3、macOS Studio ProのAUで、4秒のstereo取得がbit一致しPDC残差0 sampleを確認済みである | `kirin_local_blind_capture` clock continuity cases / `local_blind_capture_protocol` v2 shared native range / `juce_shell/tests/local_blind_capture_service_test.cpp` / `kirin_local_blind_capture_pair_comparison` / `kirin_local_blind_pdc_validation_delay` / `docs/hypha_b1_host_observation_20260907.md` |
...
| INV-S24 | ローカルBlindのGain Matchは取得開始時の利用者指定Meter Contextで固定する。通常はPOSTを0 dB基準としてPRE試聴コピーだけへ固定差分gainを適用する。PRE増幅が比較ceilingを超える場合は開始前の明示承認後だけPREを原音量に保ち、POST試聴コピーを逆差分だけ固定減衰する。両側normalize、動的leveling、無断clampは行わない。2MIXは`alignedActiveBlocksV1`の400 ms／100 ms hop、27連続active blockを変更しない。TRACK/STEMの`exactTrackEventEnergyV1`はexact 4秒を非重複20 ms窓で読み、各側最大eventから40 dB以内の対応窓を3個以上要求してenergy差の中央値を固定する。長い低レベルnoiseで短音を上書きせず、loop padding、無音、片側欠落、非有限値、範囲違い、未補正fallbackを認めない。context変更時は準備済みまたは試聴中のTrialを継続せず、通常復帰契約を維持する | `exact_track_event_policy_matches_short_and_sparse_gain` / `track_event_policy_rejects_wrong_duration_silence_and_one_sided_evidence` / `juce_shell/tests/local_blind_preparation_test.cpp` |
| INV-S28 | FREQのM/SはPOST、stereo、absolute targetでだけ成立し、同じ入力aperture、sample endpoint、layout、generationからMID `(L+R)/2`とSIDE `(L-R)/2`を一つの専用frameへ公開する。MIDはcyan実線、SIDEはviolet実線、共通0〜−96 dBFS軸で全5サイズへ表示する。M/SとΔは相互にdisabledとし、LR／MID／SIDEへ戻る操作を残す。M/SはPRE要求、PRE/POST差分、history、field、peak hold、MARK、Focus Trail、SHARPの共有channel enum、DAW保存state、Audio Thread処理を増やさない。stereoからmonoへの変更ではLRへ正規化し、monoからstereoへの復帰で自動再選択しない | `post_mid_side_is_local_even_when_a_pre_target_exists` / `mid_side_uses_one_window_and_matches_the_single_channel_definitions` / `mid_side_is_stereo_only_exclusive_and_publishes_one_coherent_frame` / `mid_side_visibility_is_post_and_stereo_only` / `verifySpectrumInteractionContract` / `verifyObservatoryViewContract` / `juce_shell/tests/ui_render_contract_test.cpp` |
| INV-S25 | ローカルBlindの製品UIはPOSTの大画面で一つだけ入口を持ち、PREは別の解析枠を消費しない。通常HeaderのMeter Contextは即時toggleではなく、2MIXをmix／master busと連続active区間、TRACK/STEMをindividual／group busと短い・疎なeventとして説明する選択menuを開く。Blind入口には取得前preflightを置き、現在context、異なるGain Match根拠、変更、明示`CAPTURE 4 S`を既存全画面のtitle／status／action内に表示して通常画面の行を増やさない。取得後は固定済みpolicyからcontext tagを表示し、channel数、名前、routing、levelから自動判定・自動変更しない。取得、準備、固定減衰の明示承認、Source 1／2の完全試聴、回答、Reveal、中断、Audio Thread確認後の通常復帰を同じ全画面UIで進める。Reveal前は割当を表示せず、背後のmeter、pair、Capture、Reference、情報操作をmouse、keyboard、accessibilityから隔離する。回答は両側の完全な一巡後だけ許可する。DAWは取得区間より前から再生でき、callbackが境界を跨いでも取得区間との交差部分だけを試聴する。完走後の停止は結果を保持し、次のSourceの明示選択で同じ区間を再度待つ。停止中の再生clock欠損は準備済み・完走済みの試聴を維持し、次の再生callbackで固定したclockを必ず検証する。途中停止・seekと再生中のclock／PDC通知の変化では試聴を失効させる。Source切替と区間端には試聴出力だけの対称5 ms遷移を置き、固定Gain、取得PCM、正本の測定とRecordを変更しない。HyphaはDAWのsolo、mute、fader、他pluginのbypass、routingを変更せず、TRACK/STEMでは対象POST出力だけを同じDAW時刻の固定コピーへ置換して残りのlive mixと共に試聴する。2MIXでは対象bus全体の固定コピーを試聴する。POSTより前で分岐したsend等を切り替えず、比較範囲をPRE／POST挿入位置間より広く主張しない。300×200への縮小、editor再表示、途中終了でも操作を失わず、自動再生、自動Reveal、自動通常復帰を行わない。Windows VST3、macOS VST3、macOS AUのexact 4秒PDC実証完了を受け、単一gateを既定ONにする | `verifyLocalBlindUiContract` / `kirin_product_entry_contract` / `juce_shell/src/PluginEditorLocalBlind.cpp` |
| INV-S26 | HyphaのJungle外観は、Kirin OS mainが検証したJungle発動とMASKING GuideのHyphaへのpublish成功が同じ利用者環境で一度ずつ成立した証明だけを初回条件とする。順序は問わず、片方だけ、INSPECT、失敗したpublish、DOM状態、ライセンスidentityだけでは発動しない。初回の連動ポップアップや由来badgeは表示せず、発動後だけ既存Displayメニューに`Jungle Mode`を出す。ON／OFFと最初の発動IDは利用者共通dataへ保存し、PRE／POST、DAW、Project Folder、製品versionを跨いで保持する。旧版はfuture schemaを上書きせず、外観の読取、JSON、排他、書込をAudio Thread、Measure Thread、FFT、Record、Referenceから隔離する | OS `hyphaJungleProjection.test.cjs` / OS `ipcRouter.test.cjs` / `KirinAppearanceContractTests` / `process_block_calls_only_rt_safe_ffi_surface` |
| INV-S27 | FREQのlive curveだけに時間方向の表示Ballisticsを持たせる。PRE／POST、LR／MID／SIDE、M/Sの絶対Spectrumは既存12 Hz描画で上昇を即時反映し、下降を20 dB／500 msとする。符号付きΔは正負の一方を保持しない150 ms対称追従とする。全5サイズで時間基準を共有し、解析snapshot、数値readout、MARK、Focus Trail、absolute history、peak hold、Audio Thread、FFT cadenceを変更しない | `verifySpectrumPresentationContract` / `verifySpectrumInteractionContract` |
| INV-S29 | 出荷JUCEで時間方向の表示filterを持つのはFREQ live curveと、MONO live curveの1秒保持（計測なしの帯域だけ、直前1秒以内の計測値を薄く描く。2026-09-26 Daisuke承認）だけで、描画だけを変え、計測・保存・読み戻しの値を変えない。絶対PRE／POST／MID／SIDEは上昇即時・下降20 dB/500 ms、符号付きΔは150 ms対称追従とする。Watchはproducerの`KirinWatchDisplay`をそのままObservatoryへ渡し、旧prototypeだけが使う`DisplaySmoother`を出荷経路へ戻さない。playback-pass／Keep MAX、Meter Session、TIME履歴、Record、Keep、`plugin_data`、FREQ数値readout、MARK、Focus Trail、peak holdはfilterを通さず、「値なし」は0.0ではなくNaNのまま運ぶ | `comparison_state_drives_the_shipping_observatory_without_a_hidden_delta_path` / `session_maximum_is_never_smoothed_toward_a_value_it_never_had` / `short_term_loudness_passes_through_as_the_exact_three_second_window` / `verifySpectrumPresentationContract` |
| INV-S30 | 六秒FIELDは、plot幅が地形の最小幅360 px未満となる100%、125%、150%の画面でPOST絶対Spectrumの背面に置き、縦軸に時間を持ち、最新の観測を下端、六秒前を上端に置く。180行は六秒の1/180であり、各行はその時刻に最も近い観測を`KIRIN_SPECTRUM_BAND_COUNT`列の全band解像度で描く。帯域の集約、観測間の補間、時間平滑を行わない。空行にするのは、最も近い観測がその系列自身のcadence（窓内の観測間隔の中央値。1行幅以上、0.5秒以下にclampする）より遠い場合に限る。cadenceを観測から測るので、hostのbuffer sizeが1024でも8192でも内部に縞を作らない。中央値は長い欠測の少数の区間に引かれないため、実際の欠測は空行として残る。ringが六秒に届かない上端も空行のままにする。濃度は band level から決め、-96..0 dBFSの全域を残したまま指数で配分するので、-6 dBFSと-60 dBFSは約4倍以上の濃度差を持つ。bandのx位置はcurveと同じ `(index+0.5)/256` 定義に一致する。Audio Threadの仕事を増やさず、既存のpaint上限を超えない | `verifyAbsoluteSpectrumContract` / `verifySixSecondFieldReadsAsTime` |
| INV-S31 | live curveでは、今の100 msで計測なしの帯域は直前1秒以内の計測値を薄く保持して描き、それより古ければ線を切る（計測値と六秒fieldは変えない）。MONOは100 msの観測ごとに、帯域ごとの「モノ加算で残るエネルギー」を測る。`M=(L+R)/2` と `S=(L-R)/2` を**時間領域で作ってから**FFTするので、位相による打ち消しが |M| に含まれる。`mono(b) = 10*log10(Pm/(Pm+Ps))`。帯域は 10 Hz〜22 kHz の対数32分割（1/3 oct）で、FREQ と同じ `log_band_edges` を使う。窓は Hann（FREQ と同一）。既知値は同相 0.00 dB、ハードパン -3.01 dB、3:1 バランス -0.97 dB。**無音（帯域電力が -120 dBFS 未満）は undefined、完全な逆相は算出床 -60 dB であり、この 2 つを混ぜない。** 測るものが無いことと、消えていることは別の事実である。host rate はリサンプルせず、観測長から `approximate_below_hz = 3*fs/frames` を出す。100 ms 観測の帯域分離能は隣接帯域が決め、音量差 20 dB で 0.07 dB、30 dB で 0.66 dB、40 dB で 4.2 dB 動く（実測）。mono 入力は全帯域 undefined。Measure Thread のみで計算し、Audio Thread には何も追加しない。常時計算する。実測（release / 100 ms 観測あたり）: MONO 単体 42〜48 us、`StereoMeter::push_observation` 全体で 208 us → 247 us（+39 us、+19%）、実時間の 0.04% 前後 | `identical_channels_lose_nothing` / `one_channel_alone_loses_three_decibels` / `a_three_to_one_balance_loses_one_decibel` / `an_inverted_pair_reaches_the_floor_and_is_not_undefined` / `silence_is_undefined_and_never_zero` / `only_the_inverted_band_drops` / `a_loud_cancellation_does_not_move_a_quieter_band` / `the_same_signal_reads_the_same_at_any_host_rate` / `mono_sum_reaches_the_snapshot_and_survives_a_changed_observation_length` / `mono_input_reports_no_mono_sum_instead_of_zero` |
| INV-S32 | SPACE の MONO 表示は、既存の表示を一切損なわずに入る場合にだけ足す。条件は MONO の帯 116 px、metric 列 136 px、残る正方形が散布図の軸ラベル `SIDE < 0` を保てる幅の 3 つで、いずれも部品自身の要求から取り、サイズ preset では判定しない（現時点では 900x600 のみ成立）。縦軸は分割スケールで、上半分が 0〜-6 dB、下半分が -6〜-24 dB。固定スケールで auto-range しない。分割位置の根拠は算術である。帯域の L/R が同極性である限り読み値は -3.01 dB（ハードパン）を下回らないので、上半分が同極性の全範囲とその余白を、下半分が打ち消しを受け持つ。**測定できない帯域は線を切る。0 dB の位置に描かない。** FIELD 側も同様で、測定できた帯域は損失 0 dB でも必ず最小の濃さを持つ。インクが無いことは「測定できなかった」だけを意味する `approximate_below_hz` は縦の区切りと `~` で示し、値は隠さない。カーブと六秒 FIELD は同じ矩形を共有せず、上がカーブ（縦=dB）、下が FIELD（縦=時間・下が現在）、周波数軸は同じ 32 band なので下に 1 本だけ置いて共有する。FIELD の行規則は `hypha::time_field` が持ち、SPACE 側で書き直さない。FIELD の濃さはカーブと同じ分割スケールから決める。履歴は GUI 側 60 観測 = 6 秒のみで、TIME 履歴にも Record にも入れない（editor を閉じると失われる）。`observed_frames` を時刻の正本とし、同一観測の重複、generation 変更、sample rate 変更、時計の逆行では最初からやり直す。PRE / POST 両 role に出る。価値判断・目標値・警告色を出さない（R-22） | `verifySpaceFieldContract` / `verifyMonoSumScale` / `verifyMonoSumHistory` / `verifyMonoSumFieldInk` / `verifyMonoSumCurveRendering` / `verifyTimeFieldContract` |
| INV-S33 | 初回surround製品範囲は役割順`L, R, C, LFE, Ls, Rs`のexact 5.1だけをLEVEL／TIME計測として受理する。6役割のPeak／True Peak／ClipをABI 16slotの先頭6枠へ保持し、残りを未測定にする。stereo固有のBAL／CORR／FIELD／MONOを捏造しない。Record／Keep、Reference、ローカルBlind、Hybrid VU、FREQ、SPACEはmono／stereo専用とし、5.1ではUI入口と内部開始gateの両方で閉じる。5.0、役割不明6ch、7.1.4をチャンネル数だけで5.1扱いしない。通常A経路のbit identityと0 sample latencyは全対応配置で維持する | `five_one_reports_every_role_without_inventing_stereo_facts` / `five_one_meter_session_keeps_role_local_peaks_and_clip_history` / `exact_five_one_stays_measurement_only_even_with_os_license` / `c_abi_channel_create` / `snapshot_layout_and_mapping_are_stable` / `verifyObservatoryViewContract` / `prepared_format_contract::verify` |
| INV-S34 | 200%と300%のPOST絶対FREQは、六秒FIELDの代わりに同じ180 frameを遠近の地形（ridge-line）として描く。最新のridgeは平面plotそのもので、xはcurveと同じ`(index+0.5)/256`、yは同じ0〜−96 dBFSとする。古いridgeほど上、中央寄り、細く、薄く描き、奥行きの縮尺は`1/(1+2.4×年齢/6秒)`、最古のridgeの床はplot上端から34%の高さ、0 dBFSの山もplot内に収める。24本のridgeはそれぞれ自分の年齢に最も近い実測frameを1枚だけ描き、そのframeが半ridge間隔（約0.13秒）より遠いridgeは描かない。時間方向の格子線は隣り合うridgeの間だけに引くので、欠測は空白のまま残り、離れた2観測の間に山を作らない。周波数方向の96列は列内に中心を持つbandの最大値を取り、狭いpeakを間引かない。ridgeは手前から奥へ描き、手前のridgeより下に隠れる奥の線・格子線・覆いは描かない（floating horizon）。ridgeの下の暗い覆いは奥を隠すためだけの面で値を持たない。地形は値を加えず、現在の値は上に重なる平面curveが示す。PRE/POST対応のΔ表示とM/Sは平面のまま地形を描かない。同じ2サイズではplotの四隅の目印、描いている量の定義（Δは`Δ(f) = POST(f) - PRE(f)  dB`、POST絶対は`L(f) = POST(f)`）、表示中の解析条件（APERTURE ms、FFT点数、256 BANDS、30 Hz）をsnapshotの値からplot左下へ描き、M/Sには描かない。Audio Threadの仕事を増やさず、既存のpaint上限を超えない | `verifyLevelLandscape` / `verifyLandscapeFrameBudget` / `verifyAbsoluteSpectrumContract` / `juce_shell/tests/ui_render_contract_test.cpp` |
| INV-S35 | 解析表示（FREQ、LIVE、SHARP、DRUM）の「入力あり」は、Watchと同じ休符の規則に従う。再生中の3秒（LUFS-Sの窓）未満の無音は休符であり、打音の間の無音ブロックで表示を消したり「INACTIVE」を出したりしない。再生停止、バイパス、Record／TRACE、3秒以上の無音ではこれまでどおり直ちに「今」の表示を消し、履歴、ロック、Session値は消さない。判定は表示専用で、計測、Record、Audio Threadの仕事を変えない | `verifyLiveInputThroughMusicalRests` |
| INV-S36 | 再生停止、または休符が3秒のWatch窓を超えたとき、六秒の履歴を持つ画面（FREQのfieldと地形、LIVE、SHARP、SPACEのMONO field）は計測済みの履歴を共通の濃さ（`stopped_history::opacity`）で暗くして残し、今の曲線と現在値だけを取り下げる。値は変えず、状態表示は履歴の上に重ねる（2026-09-26 Daisuke承認） | `verifyStoppedHistoryContract` |
| INV-S37 | 100%（300×200）と125%（375×250）では下段を上段2行目へ畳み、本体を下端の余白まで広げる。下段の操作（POST／Δ、サイズ、VU・STOP・MENU、OS Guide）は右から順に2行目に場所を保ち、domainの巡回が残りの幅を使う。POST／Δを持たないReferenceでは、その場所を巡回へ回す。状態の行（操作結果のトースト、持続する状態、実行中のCapture、フィードバックが無い間のWAITING・BYPASSEDなど短い状態）は、表示中だけ本体下端の1行の帯に全幅で出し、解析ページより手前に置く。フィードバックはクリックで詳細を開く。150%以上の下段は変えない（2026-09-26 Daisuke承認） | `verifyLayout`（`observatory_contract_test.cpp`） / `verifyFoldedFeedbackStrip` |
| INV-S38 | 100%（300×200）は見るだけの面とする。場所を取る操作（LEVELのCURRENT/MAX、TIMEの範囲とFOCUS、FREQのLR/MID/SIDE・M/S・PSB・MARK、SHARPのLR/MID/SIDE、DRUMのVIEW）は125%以上で選び、100%では位置を持たず、描かれず、クリックにもツールチップにも応じない。選んだ状態は100%でも保たれ、既定と異なるもの（MID、SIDE、MARK、HOLD、LOCK）だけを小さく名指しする。図を直接触る操作（FREQの周波数固定、DRUMの打音選択）は全サイズで同じとする。空いた場所は図と数字へ渡す：LEVELはS・I（TRACK/STEMではCrest）・MAX TP、TIME HISTORYはSとTP、DRUMは1段のHISTORYと大きな四値、FREQは操作行・凡例行・右側の絶対値軸を置かない図、SPACEは高さいっぱいの散布図と左右のBAL・CORR（2026-09-26 Daisuke承認） | `verifySpectrumControlsAt` / `verifyCompactLine` / `verifySupportedSizes` / `verifyMetricPresentationWorkflow` / `verifyPsbComposites` |

番号移行: B-887以前の文書で「INV-S25（FREQ M/S）」と記載された参照はINV-S28を指す。
