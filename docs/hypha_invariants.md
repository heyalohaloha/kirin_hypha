...
| INV-S37 | 100%（300×200）と125%（375×250）では下段を上段2行目へ畳み、本体を下端の余白まで広げる。下段の操作（POST／Δ、サイズ、VU・STOP・MENU、OS Guide）は右から順に2行目に場所を保ち、domainの巡回が残りの幅を使う。WAITING、BYPASSEDなど短い状態は表示中だけ巡回の行の右半分に出す。フィードバック（操作結果のトーストと持続する状態）と実行中のCaptureは、表示中だけ本体下端の1行の帯に全幅で出し、解析ページより手前に置き、クリックで詳細を開く。150%以上の下段は変えない（2026-09-26 Daisuke承認） | `verifyLayout`（`observatory_contract_test.cpp`） / `verifyFoldedFeedbackStrip` |
