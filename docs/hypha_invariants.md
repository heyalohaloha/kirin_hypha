...
| INV-S15 | 常設Meter SessionはRecord／Keep／Watch pass／pairing／Guide／editor lifetimeから独立し、Active音声だけで進む。Inactive／Bypassedは保持したままPauseし、明示Resetだけがgenerationを進めて統計を破棄する。currentとsession集計は同じ100ms境界を持ち、Measure worker再起動とRecord pre-roll replayでResetや二重算入を起こさない。Audio Threadには新しいlock／alloc／copyを追加しない | `starts_empty_and_only_active_audio_advances_time` / `integrated_lra_peak_and_plr_share_one_session_fact` / `live_measure_worker_advances_pauses_and_resets_independent_session` / `drain_includes_tail_off_undercounts_on_matches_full` / `b118_measure_restart_recovers_via_watchdog` / `process_block_calls_only_rt_safe_ffi_surface` |
