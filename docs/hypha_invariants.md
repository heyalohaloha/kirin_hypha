...
| INV-S30 | 六秒FIELDはPOST絶対Spectrumの背面で縦軸に時間を持ち、最新の観測を下端、六秒前を上端に置く。source 30 Hzの観測を1行ずつ、`KIRIN_SPECTRUM_BAND_COUNT`列の全band解像度で保持し、行の間引き・帯域の集約・観測間の補間・時間平滑を行わない。観測の無い行は透明のまま残し、欠測を塗りで埋めない。濃度は band level から決め、-96..0 dBFSの全域を残したまま指数で配分するので、-6 dBFSと-60 dBFSは約4倍以上の濃度差を持つ。bandのx位置はcurveと同じ `(index+0.5)/256` 定義に一致する。Audio Threadの仕事を増やさず、既存のpaint上限を超えない | `verifyAbsoluteSpectrumContract` / `verifySixSecondFieldReadsAsTime` |
