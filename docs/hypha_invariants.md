...
| INV-S32 | SPACE の MONO 表示は、既存の表示を一切損なわずに入る場合にだけ足す。条件は MONO の帯 116 px、metric 列 136 px、残る正方形が散布図の軸ラベル `SIDE < 0` を保てる幅の 3 つで、いずれも部品自身の要求から取り、サイズ preset では判定しない（現時点では 900x600 のみ成立）。縦軸は分割スケールで、上半分が 0〜-6 dB、下半分が -6〜-24 dB。固定スケールで auto-range しない。**測定できない帯域は線を切る。0 dB の位置に描かない。** FIELD 側も同様で、測定できた帯域は損失 0 dB でも必ず最小の濃さを持つ。インクが無いことは「測定できなかった」だけを意味する `approximate_below_hz` は縦の区切りと `~` で示し、値は隠さない。カーブと六秒 FIELD は同じ矩形を共有せず、上がカーブ（縦=dB）、下が FIELD（縦=時間・下が現在）、周波数軸は同じ 32 band なので下に 1 本だけ置いて共有する。FIELD の行規則は `hypha::time_field` が持ち、SPACE 側で書き直さない。FIELD の濃さはカーブと同じ分割スケールから決める。履歴は GUI 側 60 観測 = 6 秒のみで、TIME 履歴にも Record にも入れない（editor を閉じると失われる）。`observed_frames` を時刻の正本とし、同一観測の重複、generation 変更、sample rate 変更、時計の逆行では最初からやり直す。PRE / POST 両 role に出る。価値判断・目標値・警告色を出さない（R-22） | `verifySpaceFieldContract` / `verifyMonoSumScale` / `verifyMonoSumHistory` / `verifyMonoSumFieldInk` / `verifyMonoSumCurveRendering` / `verifyTimeFieldContract` |
