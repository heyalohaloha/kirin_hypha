...
| INV-S30 | 六秒FIELDはPOST絶対Spectrumの背面で縦軸に時間を持ち、最新の観測を下端、六秒前を上端に置く。180行は六秒の1/180であり、各行はその時刻に最も近い観測を、1行幅（約33 ms）以内にある場合だけ`KIRIN_SPECTRUM_BAND_COUNT`列の全band解像度で描く。帯域の集約、観測間の補間、時間平滑を行わない。hostのbuffer境界で観測間隔が1行幅を超えても内部に空行を作らず、実際の欠測と、ringが六秒に届かない上端だけが空行として残る。濃度は band level から決め、-96..0 dBFSの全域を残したまま指数で配分するので、-6 dBFSと-60 dBFSは約4倍以上の濃度差を持つ。bandのx位置はcurveと同じ `(index+0.5)/256` 定義に一致する。Audio Threadの仕事を増やさず、既存のpaint上限を超えない | `verifyAbsoluteSpectrumContract` / `verifySixSecondFieldReadsAsTime` |
