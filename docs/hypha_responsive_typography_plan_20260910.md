...
状態：P2–P3実装済み。P4の専用契約検査と変更面の同一fixture比較は実施済み、同一最終候補の全画面比較と実ホスト確認は未実施。
実装基準：`81bb2225`（B-795）の計画から開始。
...
Painterは`labelFont`、`monoFont`、`nativeTextFont`へ表示コンテキスト、文字役割、構成を渡し、返されたFontを変更せず`setFont`へ適用する。
折返しと省略は`HyphaTextStyle`のoverflow処理を使用し、Label、TextEditor、Button、ComboBox、PopupMenuにも同じ意味役割を渡す。

このAPIと合わせて、`juce_shell/src`、製品描画を使う`juce_shell/tools`、関連テストを対象にした軽量な検査をCIへ追加する。
新しいファイルと未追跡ファイルも列挙し、移行済みファイルだけを検査する方式にはしない。
検査対象には直接のFont生成、任意数値によるFont高の指定、返されたFontの変更、独自の文字圧縮やfitted-text処理を含める。
`setFont`自体は意味役割APIから得たFontに限って許可し、数値、旧定数、任意倍率を渡す経路は失敗させる。
...
P2–P3では共通コンテキスト、役割別文字契約、overflow adapter、画面台帳、全対象の移行、直接指定を防ぐsource検査と専用の`--typography-only`入口を実装した。
LEVELではLUFS主値とTP、LRA、PLR、Crestの補助値を別階層へ分け、ATTACK三指標は均等な三列の中央揃えへ統一した。通常の小文字には可読用のsecondary／tertiary色を使い、従来のmuted色は欠測、無効状態、補助線へ限定した。
Captureは物理解像度を論理寸法として再解釈せず、論理レイアウトの後に2倍描画を一度だけ適用する。Hybrid VUのREC表示はホスト録音状態にだけ従い、手動表示だけでは描かない。
配布物は更新していない。P4は変更面の同一fixture比較まで完了している。同一最終候補の全画面比較と、P5のmacOS AU / VST3およびWindows VST3実ホスト確認は最終候補で一度まとめて行う。
