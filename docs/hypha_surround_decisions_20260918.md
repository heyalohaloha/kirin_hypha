...
## 5. D-6 / D-7 / D-9 の精緻化（2026-09-18 / Daisuke レビュー由来）

**承認済み判断を覆すものではない。** 表現が広すぎた箇所と、書かれていなかった禁止を足す。

### 5.1 D-6 Correlation — overview は単一値へ畳まない

**追加**: 「overview」は**事実の集合**であり、1 つの数ではない。
7.1.4 の 5 本を平均・最小・worst pair のいずれで畳んでも、それは**新しい集約指標**であり、
D-6 の「集約を発明しない」に反する。**値と対の身元を分離しない**（集約候補 §3.1）。

Balance（B-3）にも同じ規律が効く。

### 5.2 D-6 Correlation — 対の決定性の出所

**訂正**: 「左右対は layout から一意に決まる」は広すぎた。
BS.2051-3 が定めるのは speaker position と label であって、Correlation 用の pair 集合ではない。
決定性の出所は **Hypha が置いた mirror 規則**（D-10）であり、規格ではない。

### 5.3 D-6 Balance — 「C / LFE 除外」は (B-2) の話である

**訂正**: (B-3) では「C と LFE を除外した」と書かない。
B-3 の測定量は指定された 1 対のエネルギー比であり、**C と LFE はその対の構成員ではない**。
全体エネルギーから何かを捨てて作った値ではない。

> **Balance 値には対象 pair を必ず記録する。pair 外のチャンネルはこの測定量の入力ではない。**

### 5.4 D-6 Attack — 不在を明示する

**追加**: (A-3) を採る帰結として、**初期 Nch 版に「作品全体の Attack」という集約指標は存在しない。**
欠落ではなく、発明しないという決定の結果である。

### 5.5 D-7 Downmix Observation — 候補比較の外へ出す

**構造変更**: 承認事項 3 を **3A（現行測定量の Nch 適用範囲）** と
**3B（新規サラウンド観測 / Research）** に分ける（集約候補 §6）。

Attack / Correlation / Balance は**既にある式の適用範囲**を決める作業で、対象を決めれば値が出る。
**Downmix Observation だけが一段手前にある。** 分母が未定義である以上、比較すべき候補が
並んでいない。**候補比較と呼べる段階に達していない。**

**3B を 3A の承認条件にしない。** 分けないと、初期 5.1 対応のために新しい音響指標を
急いで発明することになる。

### 5.6 D-9 downmix 係数 — 固定行列と renderer 由来 view を分ける

**追加**: Stereo Direct は「まだ見つかっていない係数」ではなく、**固定行列を持たない種類の対象**である。
未取得資料の一覧から外した（契約表 §10.6.3.1 / §10.6.5）。

> **「7.1.4 → 7.1 の係数が揃えば Atmos → stereo の行列が完成する」とは書かない。**
> 完成するのは Lo/Ro 連鎖の再現であって、Atmos の stereo 出力一般ではない。

Lo/Ro 自体も既定ではない。`Dolby downmix` という profile 名を作らず、
`Dolby Atmos Renderer / Standard (Lo/Ro) / 7.1 -> 5.1` の粒度で method identity を保つ。
