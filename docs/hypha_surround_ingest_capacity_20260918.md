...
したがって 384 MiB は、target でも絶対上限でもなく、
**scope が未解決のまま文書化された legacy budget** として扱う。
**384 MiB を維持すべき根拠は、現時点のリポジトリから復元できない。**
...
### 11.3 個別 probe の RSS 増分の算術合計（**製品 RSS ではない**）
...
## 12. 実測 — 12ch が式の 0.76 倍になる理由 [A]

### 12.1 測定自体の校正

まず probe の測定精度を確かめた（`calibrate` モード）。
ゼロ確保した f64 バッファに **1 ページあたり 1 回だけ書き込む**。

| 目標 | 確保時 | 1 ページ 1 書込後 | 比 |
|---:|---:|---:|---:|
| 64 MiB | +0.00 | +64.06 MiB | 1.001 |
| 256 MiB | +0.00 | +256.00 MiB | 1.000 |
| 949 MiB | +0.00 | +949.00 MiB | 1.000 |

**測定は正確である。** 差は被測定コード側にある。
（確保時 +0.00 は §10.3 (1) の裏付けでもある。）

### 12.2 原因 — 書き込みは planar で、`Unused` チャンネルは書かれない

```rust
vendor/ebur128/src/filter.rs:200-208
...
`chunks_exact_mut(dest_stride)` — **`audio_data` への書き込みは planar である。**
各チャンネルが連続ブロックを占める。

したがって **`Channel::Unused` のチャンネルは、そのブロックが丸ごと書かれない。**
zero page のまま残り、resident にならない。

default channel map（findings §3）で実際に使われるチャンネル数:

| ch | map | 非 Unused |
|---:|---|---:|
| 2 | `[Left, Right]` | 2 / 2 |
| 6 | `[L, R, C, Unused, LS, RS]` | 5 / 6 |
| 12 | 先頭 6 と同じ + index 6〜11 が `Unused` | **5 / 12** |

### 12.3 モデルの検証

`resident ≈ (非 Unused 比 × audio_data) + 28.0 MiB/ch`

| ch | audio_data 実書込 | その他（差） | ch 当たり | 予測 | 実測 | 誤差 |
|---:|---:|---:|---:|---:|---:|---:|
| 2 | 158.2 | 56.4 | 28.2 | 214.2 | 214.57 | **0.2%** |
| 6 | 395.5 | 166.0 | 27.7 | 563.5 | 561.54 | **0.4%** |
| 12 | 395.5 | 330.0 | 27.5 | 731.5 | 725.51 | **0.8%** |

**3 ケースとも誤差 1% 以内で一致する。** 0.76 倍は完全に説明できた。

「その他」約 28 MiB/ch は `Mode::TRUE_PEAK` の補間バッファなど、
`audio_data` 以外の per-channel state と見られる（内訳は未確認 [C]）。

### 12.4 この結果が意味すること

**findings §3 の default channel map の欠陥が、メモリ側からも観測された。**

7 チャンネル以上でチャンネルが loudness に寄与しないことは、
**そのチャンネルのフィルタ出力バッファが一度も書かれない**という形でも現れる。
**「測っていない」ことがメモリ使用量に見えている。**

逆に言えば、**明示 channel map を入れて全チャンネルを使うようにすると、
