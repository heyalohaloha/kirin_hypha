...
**B-747レビュー後の現在地：2026-09-08のB-748実装候補でReferenceの開始・保持・通常復帰を再修正した。**
[Referenceセッション構造修正計画](hypha_reference_session_repair_plan_20260907.md)を、この範囲と関連RT検証ゲートの現行計画とする。
開始中の自動減衰解除、開始状態とepochの公開競合、現行HEADの既存テスト4件失敗を修正した。
対象native・静的契約・ASan・TSan・最終sourceゲートはpassし、Windows CI実行と実DAW確認は未完了として新計画に記録した。
以下の「実装済み」「検証完了」はB-738までの履歴であり、B-747で構造修正が完了したことを意味しない。
AAX混入検査とHTML回答取込みの実装記録は維持する。
