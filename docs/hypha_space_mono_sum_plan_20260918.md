...
**判定はサイズ番号ではなく、必要な高さが取れるかどうかで行う。**
MONO の帯は 70 px（表題 + 周波数行 + 0 / -6 / -24 を分離できるプロット）、メトリクス列は 136 px
（`drawMetric` が 2 箱に要求する高さ）。両方が取れる場合にだけ MONO を足す。
結果として 600×400 と 900×600 で表示され、それ以下は従来どおりの SPACE のままになる。

散布図の軸ラベルの長短も同じ規則にした。MONO が入って正方形が縮むと `SIDE < 0` が `SIDE ...` に
切れるため、**入る幅があるかどうか**で `S<0` に切り替える。サイズ preset では判定しない。
