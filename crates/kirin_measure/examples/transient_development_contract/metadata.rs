...
impl Performance {
    pub(crate) fn density(&self) -> f64 {
        self.midi.compound_events as f64 / self.row.duration
    }
}
...
fn inspect_render(root: &Path, row: &MetadataRow) -> Result<MidiSummary, String> {
    ...
    let midi = inspect_midi(&path)?;
    if midi.raw_notes == 0 || midi.compound_events == 0 {
        return Err("empty_midi".to_string());
    }
    if midi.first_event_time_secs < -0.002 || midi.last_event_time_secs > row.duration + 0.002 {
        return Err(format!(
            "annotation_outside_duration:first={:.9};last={:.9};duration={:.9}",
            midi.first_event_time_secs, midi.last_event_time_secs, row.duration
        ));
    }
    Ok(midi)
}
