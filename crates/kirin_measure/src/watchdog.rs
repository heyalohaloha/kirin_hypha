...
use crate::{
    spawn_measure_thread, LivenessEvaluator, MeasureResult, MeterSession, MeterSessionPublication,
    RecordTakeTracker, RecordTraceQueue,
};
...
pub struct WatchdogParams {
    ...
    /// Completed-observation publication owned outside replaceable Measure workers.
    pub meter_session_publication: Option<Arc<MeterSessionPublication>>,
    ...
    pub watch_ring_cursor_epoch: Arc<AtomicU64>,
    /// 現在の ring cursor が属する full playback pass id。
    pub watch_ring_cursor_pass_id: Arc<AtomicU64>,
    /// 現在の ring 世代へ成功 push 済みの sample 数。
    pub watch_ring_cursor_samples: Arc<AtomicU64>,
    ...
    /// Audio Thread が刻む Record native clock。Measure 再起動後も同じ clock を読む。
