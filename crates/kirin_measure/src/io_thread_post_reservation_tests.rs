...
fn first_record_tick_refreshes_immediately_then_waits_one_lease_interval() {
    ...
    state.service_with(start, true, || calls.set(calls.get() + 1));
    state.service_with(start + interval() - Duration::from_nanos(1), true, || {
        calls.set(calls.get() + 1)
    });
    assert_eq!(calls.get(), 1);

    state.service_with(start + interval(), true, || calls.set(calls.get() + 1));
...
fn watch_tick_rearms_the_next_record_for_immediate_refresh() {
    ...
    let calls = Cell::new(0);
    state.service_with(start, true, || calls.set(calls.get() + 1));
    state.service_with(start + Duration::from_secs(1), false, || {
        calls.set(calls.get() + 1)
    });
    state.service_with(start + Duration::from_secs(1), true, || {
        calls.set(calls.get() + 1)
    });
...
fn watch_never_executes_a_refresh_callback() {
    let start = Instant::now();
    let mut state = ReservationLeaseRefresh::new(start);
    state.service_with(start, false, || {
...
fn failed_or_empty_refresh_attempt_is_still_throttled() {
    ...
    state.service_with(start, true, || attempts.set(attempts.get() + 1));
    state.service_with(start + Duration::from_secs(1), true, || {
