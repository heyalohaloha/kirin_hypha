...
//! Pair identity is owned by an engine-lifetime lease; the POST Watch snapshot remains separate
//! freshness evidence for measurement/readiness. Directory enumeration is deliberately absent.
...
///
/// Current claims use a dedicated pair lease that survives IO worker restart. Legacy claims have
/// no such marker and retain their former fresh Watch-snapshot proof during rolling upgrades.
pub fn pair_claim_is_owned(kirin_root: &Path, claim: &PairClaim) -> bool {
    ...
    if crate::pair_ownership_lease::pair_owner_claim_is_live(
        instance_dir,
...
pub fn publish_pair_claim(
    ...
    if let Some(current) = read_claim_path(&path) {
        ...
        // An established engine-owned pair is immutable. A later claimant is rejected; it never
        // evicts the incumbent merely because its wall-clock timestamp is newer.
...
pub fn pair_claim_owned_by_other_post(
    ...
    let Some(claim) = read_pair_claim(kirin_root, current_host_process_id(), pre_instance_id)
    else {
        return false;
    };
    pair_claim_is_owned(kirin_root, &claim)
