...
use kirin_measure::{
    ...
    publish_watch_playback_pass_boundary, record_ring_capacity_samples,
    record_window_for_record_capture, reservation, reset_watch_ring_cursor, sanitize_name,
    ...
    watch_playback_pass_should_start, watch_ring_capacity_samples, DeltaResult, LatchedPre,
    LiveLicense, LivenessEvaluator, MeasureResult, PresentationLatencySamples,
    PresentationLatencySource, RecordIngress, RecordMarkQueue, RecordStateMachine, RecordTakeBlock,
    RecordTakeTracker, RecordTraceQueue, RecordWindow, ReleaseReason, SessionSummary, SignalState,
    StoragePaths, TriggerPairResolutionFn, TriggerStopResolutionFn, WatchProducerHandoff,
    WatchdogIo, WatchdogParams, N_CHANNELS,
};
...
pub struct HyphaPost {
    ...
    /// Offline は Record 計測の継続条件にだけ使う。Record 終了権限は持たせない。
    process_mode_offline: AtomicBool,
    /// Audio-thread-local host mode edge. This survives the Keep ACK handoff so the offline
    /// boundary is producer-clocked even when Record begins a few callbacks later.
...
impl Plugin for HyphaPost {
    ...
    fn process(
        ...
        // SAFETY: single nih-plug Audio Thread; atomic pointer adoption only.
        ...
        if push_buffer {
            ...
            if recording {
                // SAFETY: producer access is confined to this callback.
                let pushed = unsafe {
