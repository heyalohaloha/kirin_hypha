...
namespace hypha::reference_audition
{
    namespace
    {
        ...
        bool parsePreparedTarget (const juce::var& value,
                                  CandidatePreparationTarget& target)
        {
            const auto* object = value.getDynamicObject();
            return object != nullptr && exactProperties (*object, {
                    "preset_id", "preset_revision_id", "check_id", "candidate_id" })
                && exactString (object->getProperty ("preset_id"), target.presetId)
                && exactString (object->getProperty ("preset_revision_id"), target.presetRevisionId)
                && exactString (object->getProperty ("check_id"), target.checkId)
                && exactString (object->getProperty ("candidate_id"), target.candidateId)
                && uuidV4Value (target.presetId) && uuidV4Value (target.presetRevisionId)
                && uuidV4Value (target.checkId) && uuidV4Value (target.candidateId);
        }

        bool sameTarget (const CandidatePreparationTarget& left,
                         const CandidatePreparationTarget& right)
        {
            return left.presetId == right.presetId
                && left.presetRevisionId == right.presetRevisionId
                && left.checkId == right.checkId && left.candidateId == right.candidateId;
        }
        ...
        bool writeImmutable (const juce::File& target, const juce::String& content)
        {
            ...
            if (bytes < 1 || bytes > maximumCandidatePreparationRequestBytes
                || ! target.getParentDirectory().createDirectory()) return false;
            if (target.existsAsFile()) return target.loadFileAsString() == content;
    ...
    CandidatePreparationTransport::CandidatePreparationTransport (juce::File transportRootIn)
        : root (std::move (transportRootIn)) {}

    juce::File CandidatePreparationTransport::requestFile (
        const juce::String& runtimeInstanceId, const juce::String& requestId) const
    {
        return runtimeIdValue (runtimeInstanceId) && uuidV4Value (requestId)
            ? root.getChildFile ("candidate_preparation_requests")
                  .getChildFile (runtimeInstanceId).getChildFile (requestId + ".json")
            : juce::File {};
    }

    juce::File CandidatePreparationTransport::acknowledgementFile (
        const juce::String& runtimeInstanceId, const juce::String& requestId) const
    {
        return runtimeIdValue (runtimeInstanceId) && uuidV4Value (requestId)
            ? root.getChildFile ("candidate_preparation_acknowledgements")
                  .getChildFile (runtimeInstanceId).getChildFile (requestId + ".json")
            : juce::File {};
    }

    std::optional<CandidatePreparationRequest> CandidatePreparationTransport::writeRequest (
        const RuntimeIdentity& identity, std::int64_t manifestRevision,
        const CandidatePreparationTarget& target, std::int64_t requestedAtMs,
        juce::String requestId) const
    {
        if (requestId.isEmpty()) requestId = juce::Uuid().toDashedString().toLowerCase();
        if (! runtimeIdValue (identity.runtimeInstanceId) || ! uuidValue (identity.workId)
            || identity.hostProcessId == 0 || ! uuidV4Value (requestId)
            || manifestRevision < 1 || manifestRevision > maxSafeJsonInteger
            || requestedAtMs < 0 || requestedAtMs > maxSafeJsonInteger
            || ! uuidV4Value (target.presetId) || ! uuidV4Value (target.presetRevisionId)
            || ! uuidV4Value (target.checkId) || ! uuidV4Value (target.candidateId)) return std::nullopt;
        auto sourcePreset = new juce::DynamicObject();
        sourcePreset->setProperty ("preset_id", target.presetId);
        sourcePreset->setProperty ("revision_id", target.presetRevisionId);
        auto selectedCandidate = new juce::DynamicObject();
        selectedCandidate->setProperty ("check_id", target.checkId);
        selectedCandidate->setProperty ("candidate_id", target.candidateId);
        auto object = new juce::DynamicObject();
        object->setProperty ("format", "kirin_hypha_reference_candidate_preparation_request");
        ...
        object->setProperty ("source_preset", juce::var (sourcePreset));
        object->setProperty ("selected_candidate", juce::var (selectedCandidate));
        const auto canonicalJson = canonicalValue (juce::var (object));
        const auto file = requestFile (identity.runtimeInstanceId, requestId);
        if (file == juce::File() || canonicalJson.isEmpty() || ! writeImmutable (file, canonicalJson))
            return std::nullopt;
        return CandidatePreparationRequest { requestId, identity, manifestRevision,
                                             requestedAtMs, target, canonicalJson };
    }

    std::optional<CandidatePreparationAcknowledgement>
    CandidatePreparationTransport::loadAcknowledgement (
        const CandidatePreparationRequest& request) const
    {
        juce::var json;
        const auto file = acknowledgementFile (request.identity.runtimeInstanceId, request.requestId);
        if (file == juce::File()
            || ! readJson (file, maximumCandidatePreparationAcknowledgementBytes, json)) return std::nullopt;
        const auto* object = json.getDynamicObject();
        if (object == nullptr || ! exactProperties (*object, {
                "format", "version", "request_id", "runtime_instance_id", "host_process_id",
                "work_id", "handled_at_ms", "outcome", "prepared_candidate", "recovery" })
            || object->getProperty ("format")
                   != "kirin_hypha_reference_candidate_preparation_acknowledgement"
            || object->getProperty ("version") != "1.0") return std::nullopt;
        CandidatePreparationAcknowledgement result;
        std::int64_t processId = 0;
        juce::String outcome;
        if (! exactString (object->getProperty ("request_id"), result.requestId)
            || ! exactString (object->getProperty ("runtime_instance_id"), result.identity.runtimeInstanceId)
            || ! exactInteger (object->getProperty ("host_process_id"), 1, 4'294'967'295, processId)
            ...
            || result.handledAtMs < request.requestedAtMs) return std::nullopt;
        result.identity.hostProcessId = static_cast<std::uint32_t> (processId);
        const auto preparedValue = object->getProperty ("prepared_candidate");
        const auto recoveryValue = object->getProperty ("recovery");
        if (outcome == "prepared")
        {
            CandidatePreparationTarget target;
            if (! recoveryValue.isVoid()
                || ! parsePreparedTarget (preparedValue, target)
                || ! sameTarget (target, request.target)) return std::nullopt;
            result.outcome = PresetSelectionOutcome::prepared;
            result.preparedCandidate = std::move (target);
        }
        else
        {
            ...
            if (recovery == nullptr || ! exactProperties (*recovery, { "reason", "action" })
                || ! exactString (recovery->getProperty ("reason"), parsed.reason)
                || ! exactString (recovery->getProperty ("action"), parsed.action)
                || ! recoveryReason (parsed.reason) || ! recoveryAction (parsed.action)) return std::nullopt;
            if (outcome == "action_required") result.outcome = PresetSelectionOutcome::actionRequired;
            else if (outcome == "rejected") result.outcome = PresetSelectionOutcome::rejected;
    ...
    bool CandidatePreparationTransport::removeExchange (
        const CandidatePreparationRequest& request) const
    {
        bool ok = true;
        const auto requestPath = requestFile (request.identity.runtimeInstanceId, request.requestId);
