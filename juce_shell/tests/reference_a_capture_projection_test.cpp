...
void testReferenceACaptureProjection(const juce::File& root,const std::shared_ptr<const ref::ACaptureData>& original,const juce::var& stat)
{
    ...
    {const juce::ScopedLock guard(lock);map.gainDb=-6;map.key="capture-version-gain";}
    require(wait([](const auto& t){return std::abs(t.binding.gainDb+6)<1e-9 && t.bins.back().b.frames==4800;}),"new display gain reuses immutable A");
    ...
    require(wait([](const auto& t){return !t.binding.source && t.capture->restored;}),"restored A cannot inherit current live alignment");
    state.revisitedWork="capture-work";access->publish(state);
    require(wait([](const auto& t){return t.binding.source && t.bins.back().b.frames==4800;}),"revisited identity may establish captured comparison");
