...
PROBE_EXPORT bool startPairPreviewProbe(std::atomic<int>* unloaded)
{
    ...
    auto* engine = kirin_hypha_create(48000, 2);
    if (!engine) return false;
    kirin_hypha_set_identity(engine, "module-preview-post", "module-preview-project", "module-preview-session", "");
    ticket.reset(kirin_hypha_pair_preview_create(engine));
    kirin_hypha_destroy(engine); // discovery owns no engine pointer
    for (int attempt=0; attempt<100; ++attempt) {
        if (hypha::pair_preview::request(ticket)) return true;
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
    }
    return false;
}
PROBE_EXPORT void cancelPairPreviewProbe() { ticket.reset(); }
PROBE_EXPORT void drainPairPreviewProbe() { ticket.reset(); kirin_hypha_pair_preview_shutdown(); }
